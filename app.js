...
async function adminWorkers(){
  ...
  shell(`<section class="content"><div class="section-title"><h1>TRABAJADORES</h1></div><div class="panel"><table><thead><tr><th>NOMBRE</th><th>RUT</th><th>CARGO</th><th>ÁREA</th></tr></thead><tbody>${(workers||[]).map(w=>`<tr><td>${esc(w.full_name)}</td><td>${esc(w.rut||"")}</td><td>${esc(w.job_title||"")}</td><td>${esc(w.site_area||"")}</td></tr>`).join("")}</tbody></table></div></section>`,true,"workers");
}
