...
async function workerDashboard(){
  stopNarration();
  const {data:rows,error}=await sb.from("module_progress").select("*").eq("user_id",currentUser.id).order("module_no");
  if(error){alert(error.message);return}
  const progress=rows||[],completed=progress.filter(x=>x.status==="approved").length;

  const cards=MODULES.map((name,i)=>{
    const n=i+1;
    const r=progress.find(x=>x.module_no===n);
    const done=r?.status==="approved";
    const unlocked=n===1||progress.some(x=>x.module_no===n-1&&x.status==="approved");
    const click=unlocked?`onclick="openWorkerModule(${n})"`:"";
    return `<div class="module ${unlocked?'':'lock'}" ${click} style="${unlocked?'cursor:pointer;':''}">
      <div class="module-top">
        <b>M${String(n).padStart(2,"0")}</b>
        <h3>${name}</h3>
      </div>
      <div class="module-body">
        <span class="badge ${done?'ok':''}">${done?'APROBADO':unlocked?'DISPONIBLE':'BLOQUEADO'}</span>
        ${r?.score!=null?` <b>${r.score}%</b>`:""}
        ${unlocked?'<div class="orange" style="margin-top:8px;font-weight:800">ABRIR MÓDULO →</div>':''}
      </div>
    </div>`;
  }).join("");
  ...
  shell(`<div class="page">
    <section class="hero">
      <div class="orange"><b>INDUCCIÓN HOMBRE NUEVO</b></div>
      <h1>BIENVENIDO, ${esc(currentProfile.full_name)}</h1>
      <p>Tu avance queda registrado en línea y asociado a tu cuenta personal.</p>
      <div style="max-width:480px">
        <div class="progress"><span style="width:${completed*10}%"></span></div>
        <p><b>${completed}/10 módulos completados</b></p>
      </div>
    </section>
    <h2>Tu ruta HSE</h2>
...
function narrate(text,onEnd){
  ...
  u.rate=0.92;
  u.pitch=1;
  courseState.speaking=true;
  const done=()=>{
    if(!courseState.speaking)return;
    courseState.speaking=false;
    if(onEnd)onEnd();
  };
  u.onend=done;
...
async function openWorkerModule(moduleNo){
  stopNarration();
  ...
  if(moduleNo!==1){
    shell(`<div class="page">
      ...
      <div class="card" style="margin-top:14px">
        <div class="orange"><b>MÓDULO ${String(moduleNo).padStart(2,"0")}</b></div>
        <h1>${MODULES[moduleNo-1]}</h1>
        <div class="notice"><b>Próxima etapa:</b> este módulo se habilitará después de validar completamente el Módulo 01 en producción.</div>
      </div>
    </div>`,false);
    return;
  }

  const [{data:module,error:me},{data:scenes,error:se},{data:progress,error:pe}]=await Promise.all([
    sb.from("lms_modules").select("*").eq("module_no",1).eq("published",true).single(),
    sb.from("module_scenes").select("*").eq("module_no",1).eq("published",true).order("scene_no"),
    sb.from("module_progress").select("*").eq("user_id",currentUser.id).eq("module_no",1).single()
  ]);

  if(me||se||pe){
    console.error(me||se||pe);
    alert("No fue posible cargar el Módulo 01. Revisa la conexión con Supabase.");
    return;
  }
  ...
  courseState={
    ...
    scenes:scenes||[],
    sceneIndex:0,
    phase: progress?.video_completed ? (progress?.recap_completed ? (progress?.case_completed ? "assessment_ready":"case"):"recap") : "objective",
    assessment:null,
    assessmentStartedAt:null,
    speaking:false
  };

  if(progress?.status==="approved") courseState.phase="approved";
  renderWorkerModule();
}

function courseStepLabel(){
  const map={
    objective:"OBJETIVO",
    teaching:"ENSEÑANZA",
    recap:"REPASO",
    case:"CASO DE TERRENO",
    assessment_ready:"EVALUACIÓN",
    assessment:"EVALUACIÓN",
    result:"RESULTADO",
    approved:"APROBADO"
  };
  return map[courseState.phase]||"MÓDULO";
}

function renderWorkerModule(){
  stopNarration();
  const m=courseState.module;
  if(!m){workerDashboard();return}

  let body="";
  if(courseState.phase==="objective") body=renderObjective();
  else if(courseState.phase==="teaching") body=renderTeaching();
  else if(courseState.phase==="recap") body=renderRecap();
  else if(courseState.phase==="case") body=renderCase();
  else if(courseState.phase==="assessment_ready") body=renderAssessmentReady();
  else if(courseState.phase==="assessment") body=renderAssessment();
  else if(courseState.phase==="approved") body=renderApproved();

  shell(`<div class="page">
    <button class="btn out" onclick="workerDashboard()">← VOLVER A MI RUTA</button>

    <section class="hero" style="margin-top:14px">
      <div class="orange"><b>MÓDULO 01 · ${courseStepLabel()}</b></div>
      <h1>${esc(m.title)}</h1>
      <p>${esc(m.objective||"")}</p>
      <div style="display:flex;gap:18px;flex-wrap:wrap;font-size:13px">
        <b>⏱ ${m.duration_minutes||15} MIN APROX.</b>
        <b>📘 ${esc(m.version||"")}</b>
        <b>🔒 PROGRESO REGISTRADO</b>
      </div>
    </section>
  ...
  if(courseState.phase==="teaching"){
    setTimeout(()=>playCurrentScene(),250);
  }
}

function renderObjective(){
  return `<div class="card">
    <div class="orange"><b>OBJETIVO DEL MÓDULO</b></div>
    <h2>Antes de comenzar</h2>
    <p style="font-size:18px;line-height:1.55">${esc(courseState.module.objective||"")}</p>
    <div class="notice">
      <b>Flujo obligatorio:</b> Enseñanza narrada → Repaso → Caso de terreno → Evaluación de 5 preguntas.
      No podrás rendir la evaluación sin completar las etapas anteriores.
    </div>
    <button class="btn" onclick="startTeaching()">▶ COMENZAR MÓDULO 01</button>
  </div>`;
}

function startTeaching(){
  courseState.sceneIndex=0;
  courseState.phase="teaching";
  renderWorkerModule();
}

function renderTeaching(){
  const s=courseState.scenes[courseState.sceneIndex];
  const total=courseState.scenes.length;
  const pct=total?Math.round(((courseState.sceneIndex)/total)*100):0;

  return `<div class="card" style="padding:0;overflow:hidden">
    <div style="background:linear-gradient(115deg,#07121a,#18313f 65%,#8b4b1d);color:white;padding:28px;min-height:430px;display:flex;flex-direction:column;justify-content:space-between">
      <div>
        <div class="orange" style="font-weight:900">ESCENA ${String(courseState.sceneIndex+1).padStart(2,"0")} DE ${String(total).padStart(2,"0")}</div>
        <h2 style="font-size:34px;margin:8px 0 16px">${esc(s?.title||"")}</h2>
        <div style="max-width:900px;font-size:20px;line-height:1.55;background:#0008;padding:20px;border-radius:12px">
          ${esc(s?.narration||"")}
        </div>
      </div>

      <div>
        <div style="font-size:22px;font-weight:900;color:#ff7a26;margin-bottom:18px">${esc(s?.visual_text||"")}</div>
        <div class="progress" style="background:#ffffff35"><span style="width:${pct}%"></span></div>
        <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;margin-top:15px;flex-wrap:wrap">
          <div><b>🔊 NARRACIÓN AUTOMÁTICA</b> · <b>NO ADELANTAR</b></div>
          <button class="btn alt" onclick="replayScene()">↻ REPETIR EXPLICACIÓN</button>
        </div>
      </div>
    </div>
  </div>`;
...
function playCurrentScene(){
  if(courseState.phase!=="teaching")return;
  const s=courseState.scenes[courseState.sceneIndex];
  if(!s)return;
  narrate(s.narration||"",async()=>{
    if(courseState.phase!=="teaching")return;
    await new Promise(r=>setTimeout(r,700));
    if(courseState.sceneIndex<courseState.scenes.length-1){
      courseState.sceneIndex++;
      renderWorkerModule();
    }else{
...
function replayScene(){
  stopNarration();
  setTimeout(()=>playCurrentScene(),150);
}

async function completeTeaching(){
  stopNarration();
  const {error}=await sb.from("module_progress").update({
    video_completed:true,
    status:"in_progress",
    updated_at:new Date().toISOString()
  }).eq("user_id",currentUser.id).eq("module_no",1);
  if(error){alert(error.message);return}
  courseState.phase="recap";
  renderWorkerModule();
}

function renderRecap(){
  return `<div class="card">
    <div class="orange"><b>REPASO VISUAL</b></div>
    <h2>Lo esencial que debes llevarte a terreno</h2>

    <div class="grid4" style="margin-top:14px">
      <div class="card"><b>HSE</b><p>Salud + Seguridad + Medio Ambiente. No es responsabilidad de una sola persona: HSE somos todos.</p></div>
      <div class="card"><b>SUPERVISIÓN</b><p>Bloqueo, doble presencia, PASSO, CdP/CdR y verificación de controles críticos en terreno.</p></div>
      <div class="card"><b>PLANIFICACIÓN</b><p>Orden y aseo, ARTP desde la planificación, revisión de bloqueo y plan de reemplazos.</p></div>
      <div class="card"><b>ANÁLISIS DE RIESGOS</b><p>ARTP y revisión de procedimientos antes de ejecutar la tarea.</p></div>
    </div>

    <div class="notice"><b>YO DIGO NO:</b> si una condición no es segura, detén, comunica y solicita apoyo. La disciplina operacional también es un control.</div>
    <button class="btn" onclick="completeRecap()">CONTINUAR AL CASO DE TERRENO →</button>
  </div>`;
}
...
async function completeRecap(){
  ...
  if(error){alert(error.message);return}
  courseState.phase="case";
  renderWorkerModule();
}

function renderCase(){
  return `<div class="card">
    <div class="orange"><b>CASO DE TERRENO</b></div>
    <h2>Estás por comenzar una tarea en faena</h2>
    <p style="font-size:18px;line-height:1.55">
      La tarea estaba planificada, pero al llegar al punto de trabajo observas que las condiciones del sector cambiaron
      y uno de los controles definidos en la ARTP ya no puede implementarse como estaba previsto.
    </p>
    <h3>¿Qué decisión representa la cultura HSE STEEL?</h3>
    <button class="btn out" style="display:block;width:100%;text-align:left;margin:8px 0" onclick="answerCase(false)">A) Continuar porque la tarea ya fue planificada.</button>
    <button class="btn out" style="display:block;width:100%;text-align:left;margin:8px 0" onclick="answerCase(true)">B) Detener, comunicar el cambio, reevaluar el riesgo y asegurar los controles antes de continuar.</button>
    <button class="btn out" style="display:block;width:100%;text-align:left;margin:8px 0" onclick="answerCase(false)">C) Ejecutar solamente la parte de la tarea que parezca más fácil.</button>
    <div id="case_feedback"></div>
  </div>`;
}

async function answerCase(correct){
  const el=$("#case_feedback");
  if(!correct){
    el.innerHTML='<div class="notice"><b>Respuesta incorrecta.</b> Una planificación previa no autoriza a trabajar cuando las condiciones cambiaron. Revisa la decisión preventiva.</div>';
    return;
  }
  el.innerHTML='<div class="notice"><b>✓ Correcto.</b> Si cambian las condiciones, se detiene, comunica, reevalúa y controla antes de continuar.</div>';
  ...
  setTimeout(()=>{
    courseState.phase="assessment_ready";
    renderWorkerModule();
  },900);
}
...
function renderAssessmentReady(){
  return `<div class="card">
    <div class="orange"><b>EVALUACIÓN MÓDULO 01</b></div>
    <h2>5 preguntas aleatorias</h2>
    <p>Las preguntas y alternativas se mezclan en cada intento. La corrección se realiza en Supabase, no en el navegador.</p>
    <div class="notice">
      <b>Criterio de aprobación:</b> mínimo 80% y ninguna pregunta crítica incorrecta.
    </div>
    <button class="btn" onclick="loadAssessment()">COMENZAR EVALUACIÓN</button>
  </div>`;
}

async function loadAssessment(){
  const holder=document.querySelector(".page");
  if(holder) holder.style.opacity=".65";

  const {data,error}=await sb.functions.invoke("get-module-assessment",{body:{module_no:1}});
  if(holder) holder.style.opacity="1";

  if(error||data?.error){
    let msg=data?.error||error?.message||"No fue posible cargar la evaluación.";
    alert(msg);
    return;
  }

  courseState.assessment=data;
  courseState.assessmentStartedAt=new Date().toISOString();
  courseState.phase="assessment";
  renderWorkerModule();
}

function renderAssessment(){
  const a=courseState.assessment;
  if(!a?.questions?.length) return renderAssessmentReady();

  const qs=a.questions.map((q,i)=>`
    <div class="card" style="margin-bottom:12px">
      <div class="orange"><b>PREGUNTA ${i+1} DE ${a.questions.length}${q.critical?' · CRÍTICA':''}</b></div>
      <h3>${esc(q.question)}</h3>
      ${q.options.map(o=>`
        <label style="display:block;padding:11px;border:1px solid #dce3e8;border-radius:8px;margin:7px 0;cursor:pointer">
          <input type="radio" name="q_${q.id}" value="${o.key}"> ${esc(o.text)}
        </label>
      `).join("")}
    </div>
  `).join("");
  ...
  return `<div>
    <div class="notice"><b>No cierres esta pantalla durante la evaluación.</b> Debes responder las 5 preguntas.</div>
    ${qs}
    <button class="btn" onclick="submitAssessment()">ENVIAR EVALUACIÓN</button>
    <div id="assessment_msg"></div>
  </div>`;
}
...
async function submitAssessment(){
  ...
  for(const q of a.questions){
    ...
    if(!selected){
      const msg=$("#assessment_msg");
      msg.innerHTML='<div class="notice"><b>Faltan respuestas.</b> Debes responder las 5 preguntas antes de enviar.</div>';
      return;
    }
    answers.push({question_id:q.id,option:selected.value});
  }

  const btn=document.querySelector('button[onclick="submitAssessment()"]');
  if(btn){btn.disabled=true;btn.textContent="CORRIGIENDO..."}

  const {data,error}=await sb.functions.invoke("submit-module-assessment",{
    body:{
      module_no:1,
      answers,
      started_at:courseState.assessmentStartedAt
    }
  });

  if(error||data?.error){
    if(btn){btn.disabled=false;btn.textContent="ENVIAR EVALUACIÓN"}
    alert(data?.error||error?.message||"No fue posible corregir la evaluación.");
    return;
  }
...
function renderAssessmentResult(result){
  ...
  const weak=(result.weak_topics||[]).map(esc).join(", ")||"Sin brechas registradas";

  shell(`<div class="page">
    <section class="hero">
      <div class="orange"><b>MÓDULO 01 · RESULTADO</b></div>
      <h1>${passed?"MÓDULO APROBADO":"REFUERZO REQUERIDO"}</h1>
      <p>Resultado registrado automáticamente en Supabase.</p>
    </section>

    <div class="grid4" style="margin-top:14px">
      <div class="card"><div class="mut">NOTA</div><div class="metric">${result.score}%</div></div>
      <div class="card"><div class="mut">ESTADO</div><div class="metric" style="font-size:22px">${passed?"APROBADO":"NO APROBADO"}</div></div>
      <div class="card"><div class="mut">FALLAS CRÍTICAS</div><div class="metric">${result.critical_failures||0}</div></div>
      <div class="card"><div class="mut">INTENTO</div><div class="metric">${result.attempts||1}</div></div>
    </div>

    <div class="notice"><b>Brechas detectadas:</b> ${weak}</div>

    ${passed
      ?'<button class="btn" onclick="workerDashboard()">VOLVER A MI RUTA HSE →</button>'
      :'<button class="btn" onclick="restartModule01()">VOLVER A REPASAR EL MÓDULO</button>'
    }
  </div>`,false);
}

function restartModule01(){
  courseState.sceneIndex=0;
  courseState.phase="recap";
  courseState.assessment=null;
  renderWorkerModule();
}

function renderApproved(){
  return `<div class="card">
    <div class="orange"><b>MÓDULO APROBADO</b></div>
    <h2>Ya completaste el Módulo 01.</h2>
    <p>Tu resultado está registrado en Supabase. Puedes volver a tu Ruta HSE.</p>
    <button class="btn" onclick="workerDashboard()">VOLVER A MI RUTA HSE</button>
  </div>`;
}
